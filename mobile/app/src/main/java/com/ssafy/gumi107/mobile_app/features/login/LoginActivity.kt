    ...
    private fun initKakaoLoginButton() {
        val callback: (OAuthToken?, Throwable?) -> Unit = { token, error ->
            if (error != null) {
                ...
                Log.d("로그인 성공", "카카오계정으로 로그인 성공 ${token.accessToken}")
                Toast.makeText(this, "로그인에 성공하였는데 뭐가문제.", Toast.LENGTH_SHORT).show()
        ...
        binding.ibLoginKakao.setOnClickListener {
            ...
            if (UserApiClient.instance.isKakaoTalkLoginAvailable(this)) {
                UserApiClient.instance.loginWithKakaoTalk(this) { token, error ->
                    if (error != null) {
                        Log.e("카톡에이피아이", "카카오톡으로 로그인 실패", error)
                        ...
                        Log.i("카톡에이피아이", "카카오톡으로 로그인 성공 ${token.accessToken}")
                    }
                }
            } else {
                UserApiClient.instance.loginWithKakaoAccount(this, callback = callback)
