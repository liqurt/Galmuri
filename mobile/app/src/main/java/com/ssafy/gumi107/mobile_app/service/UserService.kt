...
class UserService {
    ...
    fun selectUser(user : User) {
        val service = ApplicationClass.retrofit.create(UserApi::class.java)
        service.selectUser(user.userId, user.domain).enqueue(object : Callback<User> {
    ...
    fun updateUser(user: User) {
        val service = ApplicationClass.retrofit.create(UserApi::class.java)
        service.updateUser(user.userId, user.domain, user).enqueue(object : Callback<Unit> {
            override fun onResponse(call: Call<Unit>, response: Response<Unit>) {
                if (response.isSuccessful) {
    ...
    fun deleteUser(user: User) {
        val service = ApplicationClass.retrofit.create(UserApi::class.java)
        service.deleteUser(user.userId, user.domain).enqueue(object : Callback<Unit> {
            override fun onResponse(call: Call<Unit>, response: Response<Unit>) {
                if (response.isSuccessful) {
